...
// POST /api/users/:id/strava - Connect Strava
router.post('/:id/strava', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    dbHelper.data.users[userIndex].strava_connected = true;
    dbHelper.data.users[userIndex].strava_id = stravaId;
    dbHelper.data.users[userIndex].updated_at = new Date().toISOString();
    await dbHelper.write();
...
router.delete('/:id/strava', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    dbHelper.data.users[userIndex].strava_connected = false;
    dbHelper.data.users[userIndex].strava_id = null;
    dbHelper.data.users[userIndex].updated_at = new Date().toISOString();
    await dbHelper.write();
